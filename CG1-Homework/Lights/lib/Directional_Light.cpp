...
Intensity Directional_Light::lighting(const Object& inter_obj, std::vector<Object*> objs, const IntCol& intersection, const Ray& Eye) const
{
	...
	Ray light_ray(intersection.first, -direction);
	for (auto& obj : objs)
	{
		if (obj != &inter_obj)
		{
			...
			{
				Vec3 other_inter_vector = ((*other_inter).first - intersection.first);
				double other_inter_length = other_inter_vector.norm();
				if (other_inter_length  >= light_length)
					return { 0, 0, 0 };
			}
		}
	}

	std::optional<Vec3> normal = inter_obj.get_normal(intersection.first);
	if (!normal.has_value())
		return { 0, 0, 0 };
