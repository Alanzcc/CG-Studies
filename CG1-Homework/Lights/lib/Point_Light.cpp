...
Point_Light::Point_Light(Point o, Intensity lc, Point de) : origin{ o }, light_color{ lc }, decay{ de.x, de.y, de.z } {}
...
Intensity Point_Light::lighting(const Object& inter_obj, std::vector<Object*> objs, const IntCol& intersection, const Ray& Eye) const
{
	...
	for (auto& obj : objs)
	{
		if (obj != &inter_obj)
		{
			...
			{
				Vec3 other_inter_vector = ((*other_inter).first - static_cast<Vec3>(origin));
